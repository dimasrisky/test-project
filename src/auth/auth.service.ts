...
export class AuthService {
    ...
    async refreshToken(refreshTokenDto: RefreshTokenDto){
        ...
        const token = tokens.find((token: { sessionId: string; refreshToken: string; revoked: boolean; }) => token.sessionId === refreshTokenDto.sessionId && token.refreshToken === refreshTokenDto.refreshToken && token.revoked === false)
        if(!token) throw new BadRequestException('invalidRefreshToken', 'Refresh token tidak valid')
